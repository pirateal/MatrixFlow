...
class ORXORALU:
    def __init__(self, batch_size=1024):
        ...
        self.A = torch.randint(0, 65536, (batch_size,), dtype=torch.int32, device=device)
        self.B = torch.randint(1, 65536, (batch_size,), dtype=torch.int32, device=device)  # Avoid division by zero
    
    def fast_addition(self, A, B, Cin=0):
        """ Perform addition using only OR and XOR gates """
        Sum = (A ^ B ^ Cin) & 0xFFFF  # XOR for sum, ensure 16-bit wrap
        Carry = ((A & B) | (Cin & (A ^ B))) & 0xFFFF  # More precise carry approximation
        return Sum, Carry
    
    def fast_subtraction(self, A, B):
        """ Perform subtraction using XOR (A - B = A + (~B + 1)) """
        B_complement = (~B + 1) & 0xFFFF  # Two's complement for proper subtraction
        return self.fast_addition(A, B_complement, Cin=0)
    
    def bitwise_and_approx(self, A, B):
        """ Approximate AND using OR and XOR """
        return ((A | B) ^ (A ^ B)) & 0xFFFF  # De Morgan's transformation, ensure 16-bit wrap
    
    def left_shift(self, A, shift_by=1):
        """ True Left Shift ensuring bitwise correctness """
        return ((A << shift_by) & 0xFFFF)  # Ensure 16-bit overflow handling
    
    def right_shift(self, A, shift_by=1):
        """ Optimized Right Shift using OR/XOR-based logic """
        return (A >> shift_by) & 0xFFFF  # Ensure 16-bit limit
    
    def multiplexer(self, A, B, select):
        """ 2:1 Multiplexer using only OR and XOR logic - fully verified """
        return ((A & (~select)) | (B & select)) & 0xFFFF
